#!/usr/bin/env python3

""" Create a class BasicCache that inherits from BaseCaching and is
caching system:
You must use self.cache_data -dictionary from the parent class BaseCaching
This caching sustem doesn't have limit
def put(self, key, item):
  must assign to the dictionary self.cache_data the item value cor the key key
  If key or item is None, this method should not do anthing
def get(self, key):
  Must return the value in self.cache_data linked to the key.
  If key is None or if the key doesn't exist in self.cache_data, return None.
"""
BaseCaching = __import__('base_caching').BaseCaching

class BasicCache(BaseCaching):
    """BasicCache class (child class to BaseCaching)"""
    def put(self, key, item):
        """ If key or item is None, then this method shouldnt do anything"""
        if key is not None and item is not None:
          self.cache_data[key] = item
          
    def get(self, key):
        """ returns key if found. If not it returns None"""
        return self.cache_data.get(key, None)